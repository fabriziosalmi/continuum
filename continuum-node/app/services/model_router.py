    ...
    def load_models(self, path: str) -> None:
        ...
        try:
            ...
            if "models" not in config:
                raise ValueError(
                    "File di configurazione modelli non valido: manca la sezione 'models'"
                )
            ...
            for model_config in config["models"]:
                ...
                if provider_instance:
                    ...
                    print(
                        f"Configurato modello '{model_id}' con provider '{provider_type}'"
                    )
                else:
                    print(
                        f"Warning: Provider '{provider_type}' non supportato per il modello '{model_id}'"
                    )
            ...
            raise FileNotFoundError(
                f"File di configurazione modelli non trovato: {path}"
            )
        except yaml.YAMLError as e:
            raise ValueError(f"Errore nel parsing del file YAML: {e}")
        except KeyError as e:
            raise ValueError(
                f"Campo richiesto mancante nella configurazione modelli: {e}"
            )
        ...
        try:
            if provider_type.lower() == "ollama":
                ...
                if not provider.api_key:
                    print(
                        "Warning: OpenAI provider configurato ma OPENAI_API_KEY non trovata"
                    )
                    return None
                return provider
            else:
