    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "average_response_time": sum(m.response_time for m in recent_metrics) / len(recent_metrics) if recent_metrics else 0,
                "model_usage": dict(self.model_usage),
                "top_users": dict(list(self.user_activity.items())[:10]),
                "error_summary": dict(self.error_counts),
                "requests_per_minute": len([m for m in recent_metrics if m.timestamp > current_time - 60]),
            }
...
class EnhancedLogger:
    ...
    def log_request(self, endpoint: str, method: str, status_code: int, 
                   response_time: float, user_token: str, **kwargs):
    ...
    def log_model_operation(self, operation: str, model: str, user_token: str, **kwargs):
