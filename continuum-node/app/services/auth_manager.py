...
class AuthManager:
    ...
    def load_users(self, path: str) -> None:
        ...
        try:
            ...
            if "users" not in config:
                raise ValueError(
                    "File di configurazione utenti non valido: manca la sezione 'users'"
                )
            ...
            raise FileNotFoundError(
                f"File di configurazione utenti non trovato: {path}"
            )
        except yaml.YAMLError as e:
            raise ValueError(f"Errore nel parsing del file YAML: {e}")
        except KeyError as e:
            raise ValueError(
                f"Campo richiesto mancante nella configurazione utenti: {e}"
            )
    ...
    def check_rate_limit(self, token: str) -> bool:
        ...
        self.rate_limit_tracker[token] = [
            timestamp
            for timestamp in self.rate_limit_tracker[token]
