...
class ContinuumTCPServer:
    ...
    async def _handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            while True:
                ...
                try:
                    msg_type, payload = await ContinuumProtocol.unpack_message_from_stream(reader)
                    ...
                    logger.error(f"Errore nel formato del messaggio da {client_addr}: {e}")
                ...
                if msg_type == "AUTH":
                    ...
                    if "token" in payload:
                        authenticated_user = self.auth_manager.authenticate(payload["token"])
                    ...
                    await self._handle_completion(payload, writer, authenticated_user, client_addr)
                
                else:
                    logger.warning(f"Tipo di messaggio sconosciuto '{msg_type}' da {client_addr}")
                    error_response = ContinuumProtocol.create_error_response(
    ...
    async def _handle_auth(self, payload: dict, writer: asyncio.StreamWriter, client_addr) -> None:
        """
        ...
        try:
            ...
            if not token:
                response = ContinuumProtocol.create_auth_response(
                    False, 
                    "Missing token in authentication request"
                )
    ...
    async def _handle_completion(
        ...
        try:
            ...
            if not self.auth_manager.is_authorized(user.token, model):
                error_response = ContinuumProtocol.create_error_response(
                    "UNAUTHORIZED_MODEL",
                    f"User not authorized to use model: {model}"
                )
                writer.write(error_response)
                await writer.drain()
                return
            ...
            try:
                async for chunk in provider.stream_completion(messages, settings):
                    if chunk:
                        chunk_message = ContinuumProtocol.create_completion_chunk(chunk, False)
                ...
                logger.info(f"Completion completata per {user.name} con modello {model}")
