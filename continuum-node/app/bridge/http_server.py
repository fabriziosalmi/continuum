...
class HTTPServer:
        ...
        async def admin_status(token: str = Depends(self._extract_token)):
            ...
            return {
                ...
                "models": {
                    ...
                    "list": [
                        {"id": m, "provider": self.model_router.get_model_config(m)}
                ...
                "users": {
                    ...
                    "rate_limits": {
                        token: len(limits)
                        for token, limits in self.auth_manager.rate_limit_tracker.items()
                    },
                },
    ...
    def _register_routes(self) -> None:
        ...
        async def chat_completions(
            request: ChatCompletionRequest, token: str = Depends(self._extract_token)
        ):
                ...
                if request.stream:
                    ...
                    return StreamingResponse(
                        self._stream_chat_completion(
                            provider, messages, settings, request.model
                        ),
        ...
        async def websocket_chat_completions(websocket: WebSocket):
            ...
            enhanced_logger.log_websocket_event(
                "connection_opened", "unknown", client_ip=client_ip
            )
            ...
            try:
                while True:
                    ...
                    if not token:
                        ...
                        enhanced_logger.log_websocket_event(
                            "auth_failed", "no_token", reason="missing_token"
                        )
                        continue
                    ...
                    if not user:
                        ...
                        enhanced_logger.log_websocket_event(
                            "auth_failed", token, reason="invalid_token"
                        )
                        continue
                    ...
                    if not self.auth_manager.check_rate_limit(token):
                        ...
                        enhanced_logger.log_websocket_event(
                            "rate_limit_exceeded", token, user=user.name
                        )
                        continue
                    ...
                    if not self.auth_manager.is_authorized(token, model):
                        await websocket.send_json(
                            {
                                "error": {
                                    "message": f"Not authorized for model: {model}",
                                    "type": "authorization_error",
                                }
                            }
                        )
                        enhanced_logger.log_websocket_event(
                            "unauthorized_model", token, model=model, user=user.name
                        )
                        continue
                    ...
                    enhanced_logger.log_model_operation(
                        "completion_start",
                        model,
                        token,
                        user=user.name,
                ...
                enhanced_logger.log_websocket_event(
                    "connection_closed", "unknown", reason="client_disconnect"
                )
            except Exception as e:
                enhanced_logger.log_websocket_event(
                    "connection_error", "unknown", error=str(e)
                )
                try:
                    await websocket.send_json(
                        {
    ...
    async def _stream_chat_completion(
        self, provider, messages: list, settings: dict, model: str
