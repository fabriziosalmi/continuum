...
class ChatMessage(BaseModel):
    ...
    role: str = Field(
        ..., description="Il ruolo del messaggio (user, assistant, system)"
    )
...
class ChatCompletionRequest(BaseModel):
    ...
    messages: List[ChatMessage] = Field(
        ..., description="Lista dei messaggi di conversazione"
    )
    stream: Optional[bool] = Field(
        False, description="Se restituire la risposta in streaming"
    )
    temperature: Optional[float] = Field(
        None, ge=0.0, le=2.0, description="Controllo della casualità"
    )
    max_tokens: Optional[int] = Field(
        None, gt=0, description="Numero massimo di token da generare"
    )
    top_p: Optional[float] = Field(None, ge=0.0, le=1.0, description="Nucleus sampling")
    frequency_penalty: Optional[float] = Field(
        None, ge=-2.0, le=2.0, description="Penalità per la frequenza"
    )
    presence_penalty: Optional[float] = Field(
        None, ge=-2.0, le=2.0, description="Penalità per la presenza"
    )
...
class ChatCompletionChoice(BaseModel):
    ...
    finish_reason: Optional[str] = Field(
        None, description="Motivo della fine della generazione"
    )
...
class ChatCompletionResponse(BaseModel):
    ...
    choices: List[ChatCompletionChoice] = Field(
        ..., description="Lista delle scelte generate"
    )
    usage: Optional[ChatCompletionUsage] = Field(
        None, description="Informazioni sull'utilizzo"
    )
...
class ChatCompletionStreamResponse(BaseModel):
    ...
    choices: List[ChatCompletionStreamChoice] = Field(
        ..., description="Lista delle scelte in streaming"
    )
